...
//            }
            ...
            while (!usuarioEncontrado) {
                ...
                for (Integer i = 0; i < loginDoBanco.size(); i++) {
                    ...
                    if (loginDaVez.getLogin().equals(usuarioInserido)) {
                        ...
                        GeradorLog.log(TagNiveisLog.INFO, "Usuário informado: %s encontrado!".formatted(usuarioInserido), Modulo.GERAL);
                        break;
                    }
                }
                ...
                if (!usuarioEncontrado) {
                    ...
                    GeradorLog.log(TagNiveisLog.ERROR, "Usuário informado: %s não encontrado!".formatted(usuarioInserido), Modulo.GERAL);
                } else {
                    ...
                    while (true) {
                        ...
                        if (senhaDaVez.getSenha().equals(senhaInserida)) {
                            ...
                            GeradorLog.log(TagNiveisLog.INFO, "Autenticação realizada com sucesso! Usuário: %s".formatted(usuarioInserido), Modulo.GERAL);
                            return;
                        } else {
                            GeradorLog.log(TagNiveisLog.ERROR, "Tentativa falha de autenticação! Credenciais incorretas. Usuário: %s".formatted(usuarioInserido), Modulo.GERAL);
            ...
            GeradorLog.log(TagNiveisLog.ERROR, "Falha inesperada ao tentar realizar o processo de autenticação.", Modulo.GERAL);
        }
    }
